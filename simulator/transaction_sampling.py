...
def sample_providers(node_variables, K, providers):
    provider_records = node_variables[node_variables["pub_key"].isin(
        providers)]
    nodes = list(provider_records["pub_key"])
    probas = list(provider_records["degree"] /
                  provider_records["degree"].sum())
...
def sample_transactions(node_variables, amount_in_satoshi, K, eps, active_providers, verbose=False):
    ...
    if eps > 0:
        n_prov = int(eps*K)
        trg_providers = sample_providers(
            node_variables, n_prov, active_providers)
    ...
    transactions = pd.DataFrame(
        list(zip(src_selected, trg_selected)), columns=["source", "target"])
    transactions["amount_SAT"] = amount_in_satoshi
    transactions["transaction_id"] = transactions.index
    transactions = transactions[transactions["source"]
                                != transactions["target"]]
    if verbose:
        print("Number of loop transactions (removed):", K-len(transactions))
        print("Merchant target ratio:", len(
            transactions[transactions["target"].isin(active_providers)]) / len(transactions))
