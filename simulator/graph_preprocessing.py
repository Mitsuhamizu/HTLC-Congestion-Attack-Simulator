...
def prepare_edges_for_simulation(edges, amount_sat, drop_disabled, drop_low_cap, time_window=None, ts_upper_bound=None, verbose=True):
    ...
    if verbose:
        print("Total number of deleted directed channels:",
              len(edges) - len(tmp_edges))
        print("Number of remaining directed channels:", len(tmp_edges))
        print("Number of edges after aggregation: %i" %
              len(directed_aggr_edges))
...
def init_node_params(edges, providers, verbose=True):
    ...
    G = nx.from_pandas_edgelist(edges, source="src", target="trg", edge_attr=[
                                "capacity"], create_using=nx.DiGraph())
    ...
    total_capacity = pd.DataFrame(list(nx.degree(G, weight="capacity")), columns=[
                                  "pub_key", "total_capacity"])
...
def generate_graph_for_path_search(edges, transactions, amount_sat):
    ...
    G = nx.from_pandas_edgelist(all_edges, source="src", target="trg", edge_attr=[
                                "total_fee", "capacity"], create_using=nx.DiGraph())
    return G
...
def init_capacities(edges, transactions, amount_sat, all_edges, verbose=False):
    """Initialize capacity map for path search"""
    # init all edge capacity map
    tx_targets = set(transactions["target"])
    # get target
    # init capacity dict
    # edge's src and trg are key
    keys = list(zip(all_edges["src"], all_edges["trg"]))
    # edges related to trg
    is_trg = all_edges["trg"].apply(lambda x: x in tx_targets)
    # [current_cap, total_fee, is_trg, total_cap]
    vals = [list(item) for item in zip([None]*len(all_edges),
                                       all_edges["total_fee"], is_trg, all_edges["capacity"])]
    capacity_map_final = dict(zip(keys, vals))
    channels = set()
    for s, t in keys:
        if (s, t) in channels or (t, s) in channels:
            ...
            channels.add((s, t))
    edges_with_capacity = populate_capacities(
        channels, capacity_map_final, amount_sat)
    ...
    # [current_cap, total_fee, is_trg, total_cap]
    vals = [list(item) for item in zip([None]*len(edges),
                                       edges["total_fee"], is_trg, edges["capacity"])]
    ...
    for s, t in keys:
        if (s, t) in channels or (t, s) in channels:
            ...
            channels.add((s, t))
    edges_with_capacity = populate_capacities(
        channels, capacity_map, amount_sat)
    if verbose:
        print("Edges with capacity: %i->%i" %
              (len(edges), len(edges_with_capacity)))

    return capacity_map_final, edges_with_capacity
...
def populate_capacities(channels, capacity_map, amount_sat):
    ...
    for src, trg in channels:
        ...
        # get capacity if it has revert edge
        if (trg, src) in capacity_map:
            c2 = capacity_map[(trg, src)][3]
            cap = max(c1, c2)
            capacity_map[(trg, src)][3] = cap
            rnd = np.random.random()
            # for value_iter in capacity_map.values():
            #     if value_iter[0] > value_iter[3]:
            #         print("11222")
