...
def populate_route(route, k, G, router_weights=None):
    ...
    while len(path) < k+1:
        ...
        if router_weights != None:
            weights = np.array([float(router_weights.get(n, 0.0))
                                for n in neigh])
            sum_ = np.sum(weights)
            if sum_ > 0:
                probas = weights / sum_
...
class GeneticPaymentRouter():
    ...
    def _init_population(self, route, size):
        population = []
        for _ in range(size):
            success, path = populate_route(
                route, self.k, self.G, self.router_weights)
            if success:
    ...
    def _eval_population(self, population):
        ...
        pop_df = pd.DataFrame(list(zip(population, costs)),
                              columns=["route", "cost"])
    ...
    def _gen_offsprings(self, pop_df, cnt, times=5):
        ...
        rnd_indices = set(np.random.choice(
            range(L), size=len(unique_offsprings), replace=True))
